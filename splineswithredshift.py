...
for i in range(1):
    ...
    for j in range(len(flux1)): #redshift correction
    ...
    s1 = UnivariateSpline(lamcor1, flux1, k=3) #fit spline, k=3 means cubic
    ...
    fluxspline1=s1(lamcor1) #flux modeled by spline
    ...
    lamcor1a = zeros(len(flux1)) #array for +redshift in other direction
    lamcor2a = zeros(len(flux2))
    lamcor3a = zeros(len(flux3))
    lamcor4a = zeros(len(flux4))
    lamcor5a = zeros(len(flux5))
    lamcor6a = zeros(len(flux6))
    lamcor7a = zeros(len(flux7))
    lamcor8a = zeros(len(flux8))
    
    for j in range(len(flux1)):
        lamcor1a[j]=float(lam1[j])/float((1-z1m))
    for j in range(len(flux2)):
        lamcor2a[j]=float(lam2[j])/float((1-z2m))
    for j in range(len(flux3)):
        lamcor3a[j]=float(lam3[j])/float((1-z3m))
    for j in range(len(flux4)):
        lamcor4a[j]=float(lam4[j])/float((1-z4m))
    for j in range(len(flux5)):
        lamcor5a[j]=float(lam5[j])/float((1-z5m))
    for j in range(len(flux6)):
        lamcor6a[j]=float(lam6[j])/float((1-z6m))
    for j in range(len(flux7)):
        lamcor7a[j]=float(lam7[j])/float((1-z7m))
    for j in range(len(flux8)):
        lamcor8a[j]=float(lam8[j])/float((1-z8m))
        
    plt.step(lamcor1a,fluxspline1, color='g') #plot mis-corrected lambda
    plt.step(lamcor2a,fluxspline2, color='g')
    plt.step(lamcor3a,fluxspline3, color='g')
    plt.step(lamcor4a,fluxspline4, color='g')
    plt.step(lamcor5a,fluxspline5, color='g')
    plt.step(lamcor6a,fluxspline6, color='g')
    plt.step(lamcor7a,fluxspline7, color='g')
    plt.step(lamcor8a,fluxspline8, color='g')


    plt.step(lamcor1,fluxspline1, color='b') #plot corrected lambda
    plt.step(lamcor2,fluxspline2, color='b')
    plt.step(lamcor3,fluxspline3, color='b')
    plt.step(lamcor4,fluxspline4, color='b')
    plt.step(lamcor5,fluxspline5, color='b')
    plt.step(lamcor6,fluxspline6, color='b')
    plt.step(lamcor7,fluxspline7, color='b')
    plt.step(lamcor8,fluxspline8, color='b')

    plt.step(lam1,fluxspline1, color='black') #plot uncorrected lambda
    plt.step(lam2,fluxspline2, color='black')
    plt.step(lam3,fluxspline3, color='black')
    plt.step(lam4,fluxspline4, color='black')
    plt.step(lam5,fluxspline5, color='black')
    plt.step(lam6,fluxspline6, color='black')
    plt.step(lam7,fluxspline7, color='black')
    plt.step(lam8,fluxspline8, color='black')
